...
public class PlayerJump : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetButtonDown("Jump") && jumpTurns < maxJumps) {
            ...
            if(myPlayerMovement.directionMovement == FlipDirection.RIGHT)
            {
                horizontalMovement = Vector2.right/2;
            }
            else if (myPlayerMovement.directionMovement == FlipDirection.LEFT)
            {
                horizontalMovement = Vector2.left/2;
            }
